...
def scaled_pdf_est(x):
    ...
    for i in range(0, height):
        for j in range(0, width):
            ...
            est               += gaussian_kernel(sq_norm(scaled_del_space_x)) * gaussian_kernel(sq_norm(scaled_del_range_x)) / sr**2 / rr**3
    return est
...
def next_mean(x):
    ...
    for i in range(0, height):
        for j in range(0, width):
            ...
            g_val        = gaussian_kernel(sq_norm(scaled_del_x))
...
cv2.imwrite("test_gaussian_seg.png", new_rgb_img)
